...
class NabilBank(Reconcile):
    ...
    def get_phase4_df(self):
        self.bank_statement_df = pd.read_excel(Banks_FILEPATH.NABIL_BANK_FILE, skiprows=1)
        self.soa_statement_df = pd.read_csv(SOA_FILEPATH.NABIL_SOA_FILE, encoding='latin-1')
    ...
    def preprocessing_bank_stmt_phase_4(self):
        ...
        '''
        self.bank_statement_df['Date'] = pd.to_datetime(self.bank_statement_df['Date'], format="%m/%d/%Y %H:%M:%S").dt.date
