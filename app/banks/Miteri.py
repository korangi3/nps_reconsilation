...
class MiteriBank(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        self.proceed = True
        try:
            self.bank_statement_df = pd.read_excel(Banks_FILEPATH.MITERI_BANK_FILE,skiprows=1)
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.MITERI_SOA_FILE, encoding='latin-1')
        except:
            self.proceed = False
    ...
    def preprocessing_bank_stmt_phase_4(self):
        ...
        self.bank_statement_df['Desc3'] = self.bank_statement_df['Desc3'].astype(str)
        self.bank_statement_df['Desc3'] = self.bank_statement_df['Desc3'].str.rstrip('.0')
