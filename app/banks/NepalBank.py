...
class NepalBank(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        self.bank_statement_df = pd.read_excel(Banks_FILEPATH.NEPAL_BANK_FILE,skiprows=1)
        self.soa_statement_df = pd.read_csv(SOA_FILEPATH.NEPALBANK_SOA_FILE, encoding='latin-1')
    ...
    def preprocessing_bank_stmt(self):
        ...
        '''
        self.bank_statement_df['Date'] = pd.to_datetime(self.bank_statement_df['TimeStamp'], format='%d/%m/%Y %I:%M:%S %p').dt.date
    ...
    def extracting_tid_from_bank_stmt(self):
        ...
        """
        if self.bank_statement_df.empty:
            self.bank_statement_df['Transaction Id'] = None
            return
