...
class SindhubikashBank(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        self.proceed = True
        try:
            self.bank_statement_df = pd.read_excel(Banks_FILEPATH.SINDHUBIKASH_BANK_FILE,skiprows=1)
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.SINDHUBIKASH_SOA_FILE, encoding='latin-1')
        except:
            self.proceed = False
    ...
    def main(self):
        ...
        if self.proceed:
            ...
            display('No data for Sindhubikash')
    ...
    def preprocessing_bank_stmt_phase_4(self):
        ...
        '''
        display(f"bank statement{self.bank_statement_df.head(10)}")
