...
class NepalFinance(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        self.proceed = True
        try:
            self.bank_statement_df = pd.read_excel(Banks_FILEPATH.NEPAL_FINANCE_FILE,skiprows=1)
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.NEPALFINANCE_SOA_FILE, encoding='latin-1')
        except:
            self.proceed = False
    ...
    def main(self):
        ...
        if self.proceed:
            ...
            display('No data for NepalFinance')
