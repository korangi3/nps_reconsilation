...
class MuktiNathBank(Reconcile):
    ...
    def get_phase4_df(self):
        try:
            self.bank_statement_df = pd.read_excel(Banks_FILEPATH.MUKTINATH_BANK_FILE,skiprows=1)
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.MUKTINATH_SOA_FILE, encoding='latin-1')
        except:
            self.proceed = False
