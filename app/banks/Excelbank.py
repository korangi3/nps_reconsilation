...
class ExcelBank(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        self.proceed =True
        try:
            self.bank_statement_df = pd.read_excel(Banks_FILEPATH.EXCEL_BANK_FILE,skiprows=1)
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.EXCEL_SOA_FILE, encoding='latin-1')
        except:
            self.proceed = False
